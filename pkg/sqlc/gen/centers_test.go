...
func getValidCenter() CenterCreateParams {
	...
	return CenterCreateParams{
		...
		Lat:      strconv.FormatFloat(address.Lat, 'f', -1, 64),
		Long:     strconv.FormatFloat(address.Long, 'f', -1, 64),
	}
}
...
func TestCentersList(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			centers, err := testQueries.CentersList(context.Background(), tc.areaID)
			if err != nil {
